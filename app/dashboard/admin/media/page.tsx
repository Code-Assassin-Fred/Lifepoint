...
export default function MediaAdminPage() {
    ...
    const mediaList = [
        { id: '1', title: 'The Architecture of Peace', speaker: 'Steven Furtick', date: 'Yesterday', views: '1.2k', type: 'Sermon', status: 'Published' },
        { id: '2', title: 'Finding Hope Today', speaker: 'Sarah Jenkins', date: '3 days ago', views: '840', type: 'Podcast', status: 'Draft' },
        { id: '3', title: 'Faith & Mental Health', speaker: 'Dr. John', date: 'Last Week', views: '2.4k', type: 'Special', status: 'Published' },
    ];
    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-8 pb-20 pt-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4">
                <div>
                    ...
                    <p className="text-zinc-500 font-medium text-sm">
                        Total 248 items across all categories.
                    </p>
                </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 px-4">
                {[
                    { label: 'Total Views', value: '45.2k', trend: '+12%', color: 'text-teal-600' },
                    { label: 'Audio Plays', value: '18.4k', trend: '+5%', color: 'text-sky-600' },
                    { label: 'Active Drafts', value: '12', trend: '', color: 'text-slate-600' },
                            ...
                            <tbody className="divide-y divide-zinc-50">
                                {mediaList.map((item) => (
                                    <tr key={item.id} className="group hover:bg-zinc-50/50 transition-colors">
                                        ...
                                        <td className="px-8 py-5">
                                            <span className="text-sm font-bold text-zinc-600">{item.speaker}</span>
                                        </td>
                                        <td className="px-8 py-5">
                                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${item.status === 'Published' ? 'bg-green-50 text-green-600' : 'bg-zinc-100 text-zinc-400'
                                                }`}>
                                                {item.status}
                                            </span>
                                        </td>
                                        <td className="px-8 py-5 text-right">
                                            <div className="flex flex-col items-end">
                                                <span className="text-sm font-bold text-zinc-900">{item.views}</span>
